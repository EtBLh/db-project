...
$sql_query = 'SELECT * FROM `order` WHERE store = (SELECT sid FROM store WHERE owner = ?)';
// select query
if ($reqdata->filter->type == 1) {
    $sql_query = 'SELECT * FROM `order` WHERE store = (SELECT sid FROM store WHERE owner = ?) and `status` = 1';
} else if ($reqdata->filter->type == 2) {
    $sql_query = 'SELECT * FROM `order` WHERE store = (SELECT sid FROM store WHERE owner = ?) and `status` = 2';
} else if ($reqdata->filter->type == 3) {
    $sql_query = 'SELECT * FROM `order` WHERE store = (SELECT sid FROM store WHERE owner = ?) and `status` = 3';
...
if ($stmt->execute()){
    ...
    while($row = mysqli_fetch_array($result)){
        ...
        $user_stmt = $conn->prepare("SELECT `account` FROM user WHERE `uid` = ?");
        $user_stmt->bind_param("i", $row["user"]);
        if ($user_stmt->execute()) {
            $user_result = $user_stmt->get_result();
            $user_data = mysqli_fetch_array($user_result);
            $user_ac = $user_data[account];
        }
