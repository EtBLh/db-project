...
const MyShopOrders = () => {
    ...
    const getOrderList = () => {
        asyncJsonFetch("https://ubereat.nycu.me/api/get_shop_order.php",{
    ...
    const updateOrders = (action) => {
        asyncJsonFetch("https://ubereat.nycu.me/api/update_orders_status.php",{
            uid: auth.uid, token: auth.token, orders: selectedOrder.map(ord => ord.oid), action: action
        }).then(body => {
            if (body.status === 0){
                setSelectedOrder([]);
                show("update success!");
            } else show("update failed :(", "danger");
            getOrderList();
        });
    }
    ...
    return <div className="container">
        ...
        <table className="order-list"  cellSpacing="0" cellPadding="0"><tbody>
            <tr>
                <th className="useraccount">user</th>
                <th className="store-name">store</th>
        ...
        <div className="action">
            <button className="update" onClick={() => updateOrders(0)}>cancel orders</button>
            <button className="update done" onClick={() => updateOrders(1)}>finish orders</button>
        </div>
    </div>
}
